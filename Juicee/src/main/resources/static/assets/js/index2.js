...
function readAll() {
  fetch(`http://localhost:8085/Juicee/Ingredients/readAll`)
    .then((response) => {
...
function createIngredient() {
  let make = document.getElementById(`get-name`).value;
  let model = document.getElementById(`get-description`).value;
  let cost = document.getElementById(`get-imgUrl`).value;
  fetch(`http://localhost:8085/Juicee/Ingredients/create`, {
    method: `post`,
    headers: { "Content-type": "application/json" },
    body: JSON.stringify({ name: name, description: description, imgUrl: imgUrl }),
  })
...
function deleteIngredients() {
  ...
  fetch(`http://localhost:8085/Juicee/Ingredients/delete/${ids}`, {
    method: "delete",
  })
...
function readBy() {
  ...
  fetch(`http://localhost:8085/Juicee/Ingredients/readBy/${mainOption}:${subOption}`)
    .then((response) => {
      if (response.status !== 200) {
        console.error(`status: ${response.status}`);
        return;
